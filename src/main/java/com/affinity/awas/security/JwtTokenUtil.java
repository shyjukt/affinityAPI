 ...
 * Sep 26, 2016	binu.kc	Created JwtTokenUtil
 */
...
public class JwtTokenUtil implements Serializable {
    ...
    static final String CLAIM_KEY_DEVICE = "device";
    ...
    public String getDeviceFromToken(String token) {
        ...
        try {
            final Claims claims = getClaimsFromToken(token);
            device = (String) claims.get(CLAIM_KEY_DEVICE);
        } catch (Exception e) {
            device = null;
        }
        return device;
    ...
    private Boolean ignoreTokenExpiration(String token) {
        ...
        return (DEVICE_TABLET.equals(device) || DEVICE_MOBILE.equals(device));
    }

    public String generateToken(UserDetails userDetails, Device device) {
        Map<String, Object> claims = new HashMap<>();
        claims.put(CLAIM_KEY_USERNAME, userDetails.getUsername());
        claims.put(CLAIM_KEY_DEVICE, generateAudience(device));
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        ...
        return (
                username.equals(user.getUsername())
                        ...
                        && !isCreatedBeforeLastPasswordReset(created, user.getUser().getPasswordLastModified()));
    }
}